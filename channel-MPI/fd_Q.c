...
//			combine H and convective of Q
	...
	if (wall_x!=0 || wall_y!=0 || wall_z!=0 || npar>0) {
		for (id=0; id<node; id++) {
			if (id+myid*node<nsurf) {
				...
				if (q_ch>0) {
					Hsurf[iq]  +=q_ch*( 2.*(surf[ip+4]*Qsurf[iq+1]-surf[ip+3]*Qsurf[iq+2]) );
					Hsurf[iq+3]+=q_ch*( 2.*(surf[ip+2]*Qsurf[iq+4]-surf[ip+4]*Qsurf[iq+1]) );
					Hsurf[iq+1]+=q_ch*( surf[ip+4]* Qsurf[iq+3]           -surf[ip+3]* Qsurf[iq+4]           +surf[ip+2]*Qsurf[iq+2]-surf[ip+4]*Qsurf[iq]);
					Hsurf[iq+2]+=q_ch*( surf[ip+4]* Qsurf[iq+4]           +surf[ip+3]*(Qsurf[iq]+Qsurf[iq+3])+surf[ip+3]*Qsurf[iq]  -surf[ip+2]*Qsurf[iq+1]);
					Hsurf[iq+4]+=q_ch*(-surf[ip+2]*(Qsurf[iq]+Qsurf[iq+3])-surf[ip+4]* Qsurf[iq+2]           +surf[ip+3]*Qsurf[iq+1]-surf[ip+2]*Qsurf[iq+3]);
				}
				if (flag==1 && surf[ip]==1) {
					for (ii=0; ii<5; ii++) {
	...
	if (myid==root) e_tot = e_ld + e_el + e_sf + e_ch;
	MPI_Barrier(MPI_COMM_WORLD);
