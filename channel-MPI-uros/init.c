			...
			for (k=0; k<Nz; k++) {
				...
				for (j=0; j<Ny; j++) {
					for (i=0; i<Nx; i++) {
						...
						if (rand_init==0 || rand_init==1) {                     // 0: random random seed, 1: random with rand_seed
                        ...
                        } else if (rand_init==-50) {        // according to electric field
                            double ex, ey, ez, ee;
			                nx = lambda1 * n_bot[0] + lambda2 * n_top[0];
			                ny = lambda1 * n_bot[1] + lambda2 * n_top[1];
			                nz = lambda1 * n_bot[2] + lambda2 * n_top[2];
                            ...
                            ny += 0.05*randvec();
                            nz += 0.05*randvec();
                        } else if (rand_init=-15) {         // mimic laser profile
                            double dr, nr;
                            dx = (double)i - hLx + 0.5;
                            dy = (double)j - hLy + 0.5;
                            dz = (double)k - hLz;
...
//       }
        ...
        if (flow_on!=0 && myid==root) {
			for (k=0; k<Nz; k++) {
				for (j=0; j<Ny; j++) {
					for (i=0; i<Nx; i++) {
						id = (i + (j+k*Ny)*Nx);
                        if (Rho[id]==0) Rho[id] = rho; 
                    }
                }
            }
        }
