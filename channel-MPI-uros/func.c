...
{
	double S=0;
	S = 0.25*(-B_ldg+sqrt(B_ldg*B_ldg-24.*a_ldg*delT*C_ldg))/C_ldg;
	return S;
}
...
double getF0()
{
    ...
    trqq = trQQ(p);
    qqq = QQQ(p);
    eld = 0.5 * a_ldg*delT*trqq + third*B_ldg*qqq + 0.25*C_ldg*trqq*trqq ;
...
{
	FILE *grid, *qfile, *rfile, *ufile, *sfile, *ifile;
	int i, j, k, id;
