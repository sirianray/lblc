...
void cal_dQ()
{
	...
	for (iq=0, ib=0, id=0; id<lpoint; iq+=5, ib+=6, id++){
		if (info[id]==-1) {
            // determine the A coefficient in the Landau-de Gennes free energy
            if (field_on==1) Aldg = a_ldg*(delT+T_on*f_T[id]);
            else Aldg = a_ldg*delT;
			...
			for (ii=0; ii<5; ii++) {
                H[iq+ii] = -( Aldg*(*(p+ii)) + B_ldg*QQ(p,ii) + trqq*(C_ldg*(*(p+ii))-B_ldg*OneThirdDelta(ii)) - L1*(d2xQ[ii]+d2yQ[ii]+d2zQ[ii]) );
			}
            if (field_on==1) for (ii=0; ii<5; ii++) H[iq+ii]-= f_H[iq+ii];
			...
			if (flag==1) {
				qqq = QQQ(p);
                eld = 0.5*Aldg*trqq + third*B_ldg*qqq + 0.25*C_ldg*trqq*trqq;
