...
void output3(int new)
{
	...
	if (myid == root) {
		if (new!=0) {
			...
			if (patt_on!=0) pfile= fopen("pattern_3d.out","w");
			...
			if (patt_on!=0) pfile= fopen("pattern_3d.out","a");
		}
...
void write_restart()
{
	...
	if (myid == root) {
		frestart=fopen("restart.dat","w");
		
        fprintf(frestart, "%d %d %d %d %d %d\n",Nx,Ny,Nz,wall_x,wall_y,wall_z);
        for (i=0; i<Nx; i++) {
            for (j=0; j<Ny; j++) {
                id = i + j*Nx;
                fprintf(frestart,"%e %e %e\n",Q[id*5],Q[id*5+1],Q[id*5+3]);
            }
        }
        for (i=0; i<Nx; i++) {
            for (j=0; j<Ny; j++) {
...
void read_restart()
{
	int Nx0, Ny0, Nz0, wall_x0, wall_y0, wall_z0, i, j, k, id;
	FILE *frestart;
    double aa, bb, cc;
	
	if (myid == root) {
		frestart=fopen("restart.dat","r");
		
        fscanf(frestart, "%d %d %d %d %d %d\n",&Nx0,&Ny0,&Nz0,&wall_x0,&wall_y0,&wall_z0);
