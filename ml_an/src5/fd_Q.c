...
//			combine H and convective of Q
			...
			if (flow_on!=0) {
				for (ii=0; ii<5; ii++) {
					H[iq+ii] = Gamma_rot * H[iq+ii] - u[iu]*dxQ[ii] - u[iu+1]*dyQ[ii] - 0.*dzQ[ii];
				}
			} else {
				for (ii=0; ii<5; ii++) {
		...
		if (info[id]==-1) {
			...
			for(ii=0;ii<3;ii++){
				for(jj=0;jj<3;jj++){
                    ...
                    if (patt_on==0) { 
					    sigma_q[id*9+ii*3+jj] = temp - zetai * (Qmat[ii][jj] + One3rdDelta(ii,jj));
                    } else {
					    sigma_q[id*9+ii*3+jj] = temp - ((zetai-zetao) * patt_c[id] + zetao) * (Qmat[ii][jj] + One3rdDelta(ii,jj));
                    }
				}
			}
		}
