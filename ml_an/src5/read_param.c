...
{
	int ipar, i, flag;
	double x, y, z, r, Wt, W, r2, ir, K1, K2, K3, K4, K24;
...
//	fscanf(param, "L1 L2 L3 L4 %le %le %le %le\n", &L1, &L2, &L3, &L4);
	fscanf(param, "K1 K2 K3 K24 %le %le %le %le\n", &K1, &K2, &K3, &K24);
	fscanf(param, "rho %le\n", &rho);
	...
	fscanf(param, "xi %le\n", &xi);
	fscanf(param, "fr %le\n", &fr);
    fscanf(param, "zetai zetao %le %le\n", &zetai, &zetao);
...
//  calculate frank elastic modulii
    L1 = (K2+(K3-K1)*third)*0.5/(S_lc*S_lc);
    L2 = (K1-K24)/(S_lc*S_lc);
    L3 = (K3-K1)*0.5/(S_lc*S_lc*S_lc);
    L4 = (K24-K2)/(S_lc*S_lc);
    if (myid==0) printf("Elastic constants: L1=%f, L2=%f, L3=%f, L4=%f\n",L1,L2,L3,L4);
    K1 = (2.*L1 + L2 - two3rd*S_lc*L3 + L4)*S_lc*S_lc;
    K2 = (   L1      -  third*S_lc*L3     )*S_lc*S_lc*2.;
    K3 = (2.*L1 + L2 +four3rd*S_lc*L3 + L4)*S_lc*S_lc;
    K4 = L4*S_lc*S_lc;
    K24= K2+K4;
    if (myid==0) printf("mapping to : K1=%f, K2=%f, K3=%f, K24=%f\n",K1,K2,K3,K24);
