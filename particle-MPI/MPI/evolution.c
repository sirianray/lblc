		...
		for (ii=0; ii<15; ii++) {
			...
			if (wall_on!=0) {
				...
				if ( kn==-1 || kn==Nz ) {
					...
					if (k==0) {
						fout[idn] += -Bx2*Rho[ip]*(ux_bot*e[ii][0]+uy_bot*e[ii][1]);
					} else {
						fout[idn] += -Bx2*Rho[ip]*(ux_top*e[ii][0]+uy_top*e[ii][1]);
					}
				}
			}
			if (npar>0 && idn/15==ip) {
...
void cal_W()
{
	...
	for (id=0; id<lpoint*3; id+=3) {
		...
		} else if (wall_on!=0 && (id+myid*point*3)/bulk3==0) {
			W[iw+6]=  u[id]   + third*u[idzp]   - four3rd * ux_bot;
			W[iw+7]=  u[id+1] + third*u[idzp+1] - four3rd * uy_bot;
			W[iw+8]=  u[id+2] + third*u[idzp+2];
		} else if (wall_on!=0 && (id+myid*point*3)/bulk3==Nz-1) {
			W[iw+6]= -u[id]   - third*u[idzm]   + four3rd * ux_top;
...
//                        p[i*15+ii] = ( (e[ii][1] - u[i*3+1]) + 3.0 * edotu * e[ii][1] )*yforce;
			p[i*15+ii] = (e[ii][0] - u[i*3])*xforce + (e[ii][1] - u[i*3+1])*yforce + 3.0 * edotu * (e[ii][0]*xforce + e[ii][1]*yforce);
