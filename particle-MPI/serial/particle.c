...
void p_allocate()
{
	if (npar>0) {
		...
		ubounce=malloc(15*points*sizeof(double));
	}
...
void p_init()
{
	...
	for (ipar=0; ipar<npar; ipar++) {
		...
		p_W[ipar]       = 0;
		p_pos[ipar*3]   = hLx;
		p_pos[ipar*3+1] = Ly*((double)ipar+0.5)/(double)npar;
		p_pos[ipar*3+2] = hLz;
		p_rad[ipar]     = 8.01;
		p_m[ipar]       = 100.;
		p_I[ipar]		= 100.;
...
//		boundaries of search box for particle # ipar
		...
		for (z1=zlo; z1<=zhi; z1++) {
			for (y1=ylo; y1<=yhi; y1++) {
				for (x1=xlo; x1<=xhi; x1++) {
					...
					id1 = i1 + (j1+k1*Ny)*Nx;
					//	inside particle
					if (p1>0 && Q_on!=0) info[id1+bulk0]=-ipar-2;
						...
						if ( j!=3 && j!=5 && ( wall_on==0 || (z2>=0 && z2<Nz) ) ) {
							...
							if (p1 * p2 < 0) {
								...
								if (flow_on!=0) {				//	modify streaming
									nextf[id1*15+j]=id1*15+k;
									nextf[id2*15+k]=id2*15+j;
									...
									ubounce[id1*15+j] = ub_dot_e;
									ubounce[id2*15+k] =-ub_dot_e;
									nlink++;
									if (j<7) {
										vsurf[nsurf*3]  = ubx;
										vsurf[nsurf*3+1]= uby;
								...
								if (j<7 && Q_on!=0) {			//	create surface point
									...
									neighbor[sid*6+sii-1]=-qpoints*5-1;
									...
									id3 = i3 +(j3+k3*Ny)*Nx + bulk0;
									if (sii<sjj) {
										neighbor[qpoints*6+sii-1]=sid*5;
										neighbor[qpoints*6+sjj-1]=id3*5;
									} else {
										neighbor[qpoints*6+sjj-1]=sid*5;
										neighbor[qpoints*6+sii-1]=id3*5;
									}
									...
									if (sii!=1 && sjj!=1) {
										...
										if (onsurface(i3,j3,k3,ipar)==-1) {
											neighbor[qpoints*6]  =-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
											i3 = si + di;
											if(i3<0)   i3 += Nx;
											if(i3>=Nx) i3 -= Nx;
											neighbor[qpoints*6+1]= 5*(i3 +(j3+k3*Ny)*Nx + bulk0);
										} else {
											i3 = si + di;
											if(i3<0)   i3 += Nx;
											if(i3>=Nx) i3 -= Nx;
											...
											if(i3<0)   i3 += Nx;
											if(i3>=Nx) i3 -= Nx;
											neighbor[qpoints*6+1]=-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
										}
									}
									...
									if (sii!=3 && sjj!=3) {
										...
										if (onsurface(i3,j3,k3,ipar)==-1) {
											neighbor[qpoints*6+2]=-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
											j3 = sj + di;
											if(j3<0)   j3 += Ny;
											if(j3>=Nx) j3 -= Ny;
											neighbor[qpoints*6+3]= 5*(i3 +(j3+k3*Ny)*Nx + bulk0);
										} else {
											j3 = sj + di;
											if(j3<0)   j3 += Ny;
											if(j3>=Nx) j3 -= Ny;
											...
											if(j3<0)   j3 += Ny;
											if(j3>=Nx) j3 -= Ny;
											neighbor[qpoints*6+3]=-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
										}
									}
									...
									if (sii!=5 && sjj!=5) {
										...
										if (onsurface(i3,j3,k3,ipar)==-1) {
											neighbor[qpoints*6+4]=-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
											k3 = sk + di;
											if (wall_on==0) {
												if(k3<0)   k3 += Nz;
												if(k3>=Nz) k3 -= Nz;
											}
											neighbor[qpoints*6+5]= 5*(i3 +(j3+k3*Ny)*Nx + bulk0);
										} else {
											k3 = sk + di;
											if (wall_on==0) {
												if(k3<0)   k3 += Nz;
											...
											neighbor[qpoints*6+5]=-5*(i3 +(j3+k3*Ny)*Nx + bulk0)-1;
										}
									}
									qpoints++;									
								}
	...
	printf("nlink=%d, nusrf=%d, qpoints=%d\n",nlink,nsurf,qpoints);
}
