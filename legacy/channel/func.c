...
void cal_feqf(real f_eq0[Nx][Ny][Nz][15], real f[Nx][Ny][Nz][15], real sigma[Nx][Ny][Nz][3][3])
{
		  int i,j,k,ii=0,jj=0,kk=0;
		  ...
		  //	real u0[Nx][Ny][Nz][3];
		  //	real Rho0[Nx][Ny][Nz];
		  real mom0[Nx][Ny][Nz][3];
		  real temp1, temp2;

		  //	cal_rho(Rho0, f);
		  //	cal_u(u0, f);
		  if(compressibility!=0)cal_mom(mom0, f);
		  ...
		  for (i=0; i<Nx; i++) {
					 for (j=0; j<Ny; j++) {
								for (k=0; k<Nz; k++) {
										  ...
										  //				printf("%21.16f %21.16f\n",A0+14.0*A2,Rho0[i][j][k]);
										  ...
										  for(ii=0;ii<15;ii++){
													 ...
													 if(ii==0){
																...
																for(jj=0;jj<3;jj++){
																		  for(kk=0;kk<3;kk++){
																					 temp+=8.0*E[jj][kk]*(double)(e[ii][jj]*e[ii][kk]);
																		  }
																}
													 }
													 else {
																...
																for(jj=0;jj<3;jj++){
																		  for(kk=0;kk<3;kk++){
																					 temp+=E[jj][kk]*(double)(e[ii][jj]*e[ii][kk]);
																		  }
																}
													 }
													 ...
													 for(jj=0;jj<3;jj++){
																if (compressibility==1) {
																		  temp += (B*mom0[i][j][k][jj]*e[ii][jj]+C*mom0[i][j][k][jj]*mom0[i][j][k][jj]/Rho[i][j][k])/rho;
																} else {
																		  temp += B*u[i][j][k][jj]*e[ii][jj]+C*u[i][j][k][jj]*u[i][j][k][jj];
																}

																for(kk=0;kk<3;kk++){
																		  if (compressibility==1) {
																					 temp += (D*mom0[i][j][k][jj]*mom0[i][j][k][kk]*e[ii][jj]*e[ii][kk]/Rho[i][j][k])/rho;
																		  } else {
																					 temp += D*u[i][j][k][jj]*u[i][j][k][kk]*e[ii][jj]*e[ii][kk];
																		  }

																}
													 }
													 f_eq0[i][j][k][ii] = temp*rho;
										  }
...
void cal_feqf_new(real f_eq0[Nx][Ny][Nz][15], real f[Nx][Ny][Nz][15], real sigma[Nx][Ny][Nz][3][3])
{
		  ...
		  for (i=0; i<Nx; i++) {
			...
			if(1==0){
					rhototal = rho*(real)(Nx*Ny*Nz)/rhototal;
					for (i=0; i<Nx; i++) {
							 for (j=0; j<Ny; j++) {
										for (k=0; k<Nz; k++) {
													for(ii=0;ii<15;ii++) f_eq0[i][j][k][ii] *= rhototal;
										}
							 }
					}
			}
...
// This is the portion of stress that go into forcing term
																		  if(ii>0 && ii<7){
																					 p[i][j][k][ii] += 0 * T1 * (detau[i][j][k][0]*e[ii][0] +detau[i][j][k][1]*e[ii][1] +detau[i][j][k][2]*e[ii][2] ); 
																		  }
																		  else if (ii>6){
																					 p[i][j][k][ii] += 0 * T2 * (detau[i][j][k][0]*e[ii][0] +detau[i][j][k][1]*e[ii][1] +detau[i][j][k][2]*e[ii][2] );
																		  }
																}
													 }
										  }
...
void cal_u(real u0[Nx][Ny][Nz][3], real f[Nx][Ny][Nz][15])
{
		  ...
		  for(i=0;i<Nx;i++){
					 for(j=0;j<Ny;j++){
								for(k=0;k<Nz;k++){
										  u0[i][j][k][0]=u0[i][j][k][0]/Rho[i][j][k];
										  u0[i][j][k][1]=u0[i][j][k][1]/Rho[i][j][k];
										  u0[i][j][k][2]=u0[i][j][k][2]/Rho[i][j][k];
								}
					 }
		  }
...
void monitor(int flag, int print)
{
		  ...
		  for(i=0;i<Nx;i++){
					 for(j=0;j<Ny;j++){
								for(k=0;k<Nz;k++){
										  ...
										  if(!cav_on || (cav_on && cav_flag[i][j][k]>-1) ){
													 ...
													 k_eng_new+=0.5*(u[i][j][k][0]*u[i][j][k][0]+u[i][j][k][1]*u[i][j][k][1]+u[i][j][k][2]*u[i][j][k][2])*Rho[i][j][k];
													 utotal[0]+=u[i][j][k][0];
													 utotal[1]+=u[i][j][k][1];
													 utotal[2]+=u[i][j][k][2];
													 rhototal+=Rho[i][j][k];
		  ...
		  if(print!=0)printf("vel=%20.15f %20.15f %20.15f\n",utotal[0], utotal[1], utotal[2]);
		  if(print!=0)printf("rho=%20.15f\n",rhototal);

		  if(flag<=0){
					 if(print!=0) printf("dif_Q^2=%30.26f\n",q_diff_sq);
					 if(q_diff_sq != q_diff_sq){
								printf("Nan detected in monitor\n");
							  	exit(1);
					 }
		  }

		  if(q_diff_sq < 5e-20 && flag<=0) Q_converge=1;
...
{
		  int i, j, k,  ip, jp, kp, im, jm, km;
		  FILE *qfile, *ufile, *grid, *sfile, *dfile;
		  real dx, dy, dz, rsq;

		  if(input){
					 qfile=fopen("Q.out","w");
					 ufile=fopen("vel.out","w");
		  ...
		  for(k=0;k<Nz;k++){
					 for(j=0;j<Ny;j++){
								for(i=0;i<Nx;i++){
										  ...
										  fprintf(sfile,"%e %e %e\n",detau[i][j][k][0],detau[i][j][k][1],detau[i][j][k][2]);
										  fprintf(dfile,"%e\n",Rho[i][j][k]);

										  if(input) fprintf(grid,"%d %d %d %d\n",i,j,k,cav_flag[i][j][k]);
								}
					 }
		  }
...
void check(real Q0[Nx][Ny][Nz][3][3])
{
		  ...
		  real c1=eight3rd, c2=four3rd, c3=third, dx, dy, dz, dr2i;
		  real a0, a1, a2, a3, a4;
		  ...
		  xfile=fopen("xcheck","w");
		  yfile=fopen("ycheck","w");
		  zfile=fopen("zcheck","w");
		  ...
		  for (i=0; i<Nx; i++) {
					 for (j=0; j<Ny; j++) {
								for (k=0; k<Nz; k++) {
										  ...
										  if(k==0 && wall_anchoring!=0){
													 d2z0 = c1*Qd[0] + c2*Qu[0] - 4.0 * a0;
													 d2z1 = c1*Qd[1] + c2*Qu[1] - 4.0 * a1;
													 d2z2 = c1*Qd[2] + c2*Qu[2] - 4.0 * a2;
													 d2z3 = c1*Qd[3] + c2*Qu[3] - 4.0 * a3;
													 d2z4 = c1*Qd[4] + c2*Qu[4] - 4.0 * a4;
													 t12  =-c2*Qd[0] + c3*Qu[0] +       a0;
													 t22  =-c2*Qd[1] + c3*Qu[1] +       a1;
													 t32  =-c2*Qd[2] + c3*Qu[2] +       a2;
													 t42  =-c2*Qd[3] + c3*Qu[3] +       a3;
													 t52  =-c2*Qd[4] + c3*Qu[4] +       a4;
										  }
										  else if(k==Nz-1 && wall_anchoring!=0){
													 d2z0 = c1*Qu[0] + c2*Qd[0] - 4.0 * a0;
													 d2z1 = c1*Qu[1] + c2*Qd[1] - 4.0 * a1;
													 d2z2 = c1*Qu[2] + c2*Qd[2] - 4.0 * a2;
													 d2z3 = c1*Qu[3] + c2*Qd[3] - 4.0 * a3;
													 d2z4 = c1*Qu[4] + c2*Qd[4] - 4.0 * a4;
													 t12  = c2*Qu[0] - c3*Qd[0] -       a0;
													 t22  = c2*Qu[1] - c3*Qd[1] -       a1;
													 t32  = c2*Qu[2] - c3*Qd[2] -       a2;
													 t42  = c2*Qu[3] - c3*Qd[3] -       a3;
													 t52  = c2*Qu[4] - c3*Qd[4] -       a4;
										  }
										  else {
										  ...
										  if        (i==0) tempd=Rho[1][j][k]    - Rho[0][j][k];
										  else if (i==Nx-1) tempd=Rho[Nx-1][j][k] - Rho[Nx-2][j][k];
										  else             tempd=0.5*(Rho[i+1][j][k] - Rho[i-1][j][k]);
										  mcheck[i][j][k][0] = tempd*T + kappa*(2.0*(t10*d2Q[0][0]+t20*d2Q[0][1]+t30*d2Q[0][2]+t40*d2Q[1][1]+t50*d2Q[1][2])+t10*d2Q[1][1]+t40*d2Q[0][0]);

										  if        (j==0) tempd=Rho[i][1][k] - Rho[i][0][k];
										  else if (j==Ny-1) tempd=Rho[i][Ny-1][k] - Rho[i][Ny-2][k];
										  else             tempd=0.5*(Rho[i][j+1][k] - Rho[i][j-1][k]);
										  mcheck[i][j][k][1] = tempd*T + kappa*(2.0*(t11*d2Q[0][0]+t21*d2Q[0][1]+t31*d2Q[0][2]+t41*d2Q[1][1]+t51*d2Q[1][2])+t11*d2Q[1][1]+t41*d2Q[0][0]);

										  if        (k==0) tempd=Rho[i][j][1] - Rho[i][j][0];
										  else if (k==Nz-1) tempd=Rho[i][j][Nz-1] - Rho[i][j][Nz-2];
										  else             tempd=0.5*(Rho[i][j][k+1] - Rho[i][j][k-1]);
										  mcheck[i][j][k][2] = tempd*T + kappa*(2.0*(t12*d2Q[0][0]+t22*d2Q[0][1]+t32*d2Q[0][2]+t42*d2Q[1][1]+t52*d2Q[1][2])+t12*d2Q[1][1]+t42*d2Q[0][0]);
		  ...
		  for(i=(Nx+1)/2;i<(Nx+3)/2;i++){
					 for(j=0;j<Ny;j++){
								for(k=0;k<Nz;k++)	fprintf(xfile,"%e ",mcheck[i][j][k][0]);
								fprintf(xfile,"\n");
					 }
					 fprintf(xfile,"\n");
		  }
		  ...
		  for(i=(Nx+1)/2;i<(Nx+3)/2;i++){
					 for(j=0;j<Ny;j++){
								for(k=0;k<Nz;k++)	fprintf(yfile,"%e ",mcheck[i][j][k][1]);
								fprintf(yfile,"\n");
					 }
					 fprintf(yfile,"\n");
		  }
		  ...
		  for(i=(Nx+1)/2;i<(Nx+3)/2;i++){
					 for(j=0;j<Ny;j++){
								for(k=0;k<Nz;k++)	fprintf(zfile,"%e ",mcheck[i][j][k][2]);
								fprintf(zfile,"\n");
					 }
					 fprintf(zfile,"\n");
		  }
...
void cav_surf(){
		  ...
		  if(cav_on){
					 for(i=0;i<Nx;i++){
								for(j=0;j<Ny;j++){
										  for(k=0;k<Nz;k++){
													 if( (j==start && k<=h) || (j==stop && k<=h) || (j>=start && j<=stop && k==h) ) cav_flag[i][j][k] = 1;
													 if( j>start && j<stop && k<h) cav_flag[i][j][k] = -1;
										  }
								}
					 }
		  }
