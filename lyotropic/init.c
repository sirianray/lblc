...
void init()
{
	int i, j, k, ii, ip, id, ipar, jj, ndrop;
	time_t t;
	...
	if (newrun_on!=0) {
		if (myid==0) {
            //  for droplets
            if (rand_init==-100) {
                param  = fopen("drops.in","r");
                fscanf(param, "ndrop %d\n", &ndrop);
		        drop   = malloc(3*ndrop*sizeof(real));
		        rdrop2 = malloc(  ndrop*sizeof(real));
                for (i=0; i<ndrop; i++) {
                    fscanf(param,"%le %le %le %le\n", &drop[i*3], &drop[i*3+1], &drop[i*3+2], &rdrop2[i]);
                    rdrop2[i] = rdrop2[i]*rdrop2[i];
                    printf("%f %f %f %f\n",drop[i*3],drop[i*3+1],drop[i*3+2],rdrop2[i]); 
                }
                fclose(param);
            }
			...
			for (k=0; k<Nz; k++) {
				...
				for (j=0; j<Ny; j++) {
					for (i=0; i<Nx; i++) {
						...
						if (rand_init==0 || rand_init==1) {                     // 0: random random seed, 1: random with rand_seed
                        ...
                        } else if (rand_init==-321) {        // defect annihilation, defect line parallel to far-field director
                            double cx, cy, b, th;
                            cx = (double)Nx*0.5;
                            cy = (double)Ny*0.5;
                            b  = q_init;
                        ...
                        } else if (rand_init==-411) {        // nematic droplet with smoother director field
                            double dx, dy, dz, dr, dw;
                            nx = 1.;
                            ny = 0.;
                            nz = 0.;
                            dx = (double)i - Nx/2;
                            ...
                            dz = (double)k - Nz/2;
                            dr = sqrt(dy*dy+dz*dz);
                            if (dr>0 && dx*dx+dy*dy+dz*dz<q_init*q_init) {
                                dw = 0.5*((q_init*q_init-dx*dx)/dr+dr);
                                nx = dw/(dw*dw+dx*dx);
                                ny =-dx/(dw*dw+dx*dx)*dy/dr;
                                nz =-dx/(dw*dw+dx*dx)*dz/dr;
                            }
                        } else if (rand_init==-412) {       // 1 nematic tactoid; long axis = rand_seed; q_init = aspect ratio (>=1)
                            double cx, cy, cz, d, b, c, R, Rsq, a, tx, ty, tz, dx, dy, dz, dr, dr2, dw;
                            cx = 0.5*(double)Nx - 0.5;
                            cy = 0.5*(double)Ny - 0.5;
                            cz = 0.5*(double)Nz - 0.5;
                            d  = (double)rand_seed*0.5;     // semi-long-axis
                            b  = d/q_init;                  // semi-short-axis
                            R  = (b+d*d/b)*0.5;             // radius of curvature for outline
                            Rsq= R*R;
                            c  = R-b;                       // y-displacement of center of curvature
                            tx = cx;
                            ty = cy;
                            tz = cz;
                            ...
                            dz = (double)k - tz;
                            if (dy*dy+dz*dz>0) {
                                ir  = sqrt(1./(dy*dy+dz*dz));
                                dy += dy*ir*c;
                                dz += dz*ir*c;
                                dr2 = dx*dx+dy*dy+dz*dz;
                                if (dr2<Rsq) {
                                    dr = sqrt(dr2);
                                    dw = 0.5*((d*d-dx*dx)/dr+dr);
                                    nx = dw/(dw*dw+dx*dx);
                                    ny =-dx/(dw*dw+dx*dx)*dy/dr;
                                    nz =-dx/(dw*dw+dx*dx)*dz/dr;
                                }
                            }
                        } else if (rand_init==-421) {       // 1 off-center radial droplet, radius = q_init
                            double cx, cy, cz, dx, dy, dz, dr2;
                            nx = 1.;
                            ny = 0.;
                            nz = 0.;
                            ...
                            cy = 0.5*(double)Ny - 0.5;
                            cz = 0.5*(double)Nz - 0.5;
                            dx = (double)i - cx + q_init;
                            dy = (double)j - cy;
                            dz = (double)k - cz;
                            dr2= dx*dx+dy*dy+dz*dz;
                            if (dr2<=q_init*q_init) {
                                if (dr2>0) {
                                    nx = dx;
                                    ny = dy;
                                    nz = dz;
                        ...
                        } else if (rand_init==-431) {       // 2 nematic droplets with homeotropic anchoring, radius = q_init
                            double dx, dy, dz, dr2;
                            nx = 1.;
                            ny = 0.;
                            nz = 0.;

                            dx = (double)i - Nx/2 - q_init; 
                            dy = (double)j - Ny/2;
                            dz = (double)k - Nz/2;
                            dr2= dx*dx+dy*dy+dz*dz;
                            if (dr2<=q_init*q_init) {
                        ...
                        } else if (rand_init==-432) {       // 2 nematic tactoids withi smooth director field. rand_seed=long diameter, q_init=aspect ratio (>1)
                            double dx, dy, dz, dr, dr2, dw, q_init2, rr, rr2;
                            q_init2 = q_init*q_init;
                            rr = 0.5*(double)rand_seed;
                            rr2= rr*rr;
                            nx = 1.;
                            ny = 0.;
                            nz = 0.;
                            ...
                            dz = (double)k - Nz/2;
                            dr2= dx*dx+dy*dy+dz*dz;
                            if (dx*dx+(dy*dy+dz*dz)*q_init2<=rr2) {
                                if (dr2>0) {
                                    dr = sqrt(dr2);
                                    dw = 0.5*((rr2-dx*dx)/dr+dr);
                                    nx = dw/(dw*dw+dx*dx);
                                    ny =-dx/(dw*dw+dx*dx)*dy/dr;
                                    nz =-dx/(dw*dw+dx*dx)*dz/dr;
                                }
                        ...
                        } else if (rand_init==-433 || rand_init==-434 || rand_init==-435) {    // 2 nematic tactoids coalescence; long axis = rand_seed; q_init = aspect ratio (>=1)
                            double cx, cy, cz, d, b, c, R, Rsq, a, tx, ty, tz, dx, dy, dz, dr, dr2, dw;
                            cx = 0.5*(double)Nx - 0.5;
                            cy = 0.5*(double)Ny - 0.5;
                            cz = 0.5*(double)Nz - 0.5;
                            ...
                            if (rand_init==-433) {          // tactoids align in x
                                tx = cx - d;
                                ty = cy;
                            } else if (rand_init==-434) {   // tactoids align in y
                                tx = cx;
                                ty = cy - b;
                            } else {                        // tactoids align in 45d
                                a  = (sqrt(R*(R+2.*b)-b*b)-c)*0.5;
                                tx = cx - a;
                                ty = cy - a;
                            }
                ...
                for (k=0; k<Nz; k++) {
                	for (j=0; j<Ny; j++) {
                		for (i=0; i<Nx; i++) {
                            ...
                            if (rand_init==-301) {          // pure droplet, no Q. q_init is the radius. q_init>0 means phi=1 inside droplet; q_init<0 means phi=0 inside droplet
                                double cx, cy, cz, dx, dy, dz, rc2, phi_droplet;
                				cx = hLx - 0.5;
                				cy = hLy - 0.5;
                				cz = hLz ;
                				dx = (double)i - cx;
                                ...
                                rc2= q_init*q_init;
                                phi_droplet = 1.;
                                if (q_init<0) phi_droplet = 0.;
                                if (dx*dx+dy*dy+dz*dz<=rc2) ly_phi[id] = phi_droplet;
                                else ly_phi[id] = 1.-phi_droplet;
                                ly_mu[id] = 0.;
                            } else if (rand_init==-300) {       // uniform phi, no Q
                                ly_phi[id] = q_init;
                                ly_mu[id]  = 0.;
                            } else if (rand_init==-302) {       // flat interface (in x), no Q
                                if (i>=(int)(Nx/2)) ly_phi[id] = 0.;
                                else ly_phi[id] = 1.;
                                ly_mu[id] = 0.;
                            } else if (rand_init==-303) {       // flat interface (in z) set by q_init, can be seen as coarse-grained Youngki's system
                                if (k>=q_init) ly_phi[id] = 0.;
                                else ly_phi[id] = 1.;
//                                ly_phi[id] = 1.;
                                ly_mu[id] = 0.;
                            } else if (rand_init==-304) {       // cubic box
                                double cx, cy, cz, dx, dy, dz, rc2, phi_droplet;
                				cx = hLx - 0.5;
                				cy = hLy - 0.5;
                				cz = hLz ;
                                ...
                                if (fabs(dx)<q_init && fabs(dy)<q_init && fabs(dz)<q_init) ly_phi[id]=1.;
                                else ly_phi[id]=0.;
                                ly_mu[id]=0.;
                            } else if (rand_init==-100) {       // Youngki's system
                                double dx, dy, dz;
                                if ((double)k<q_init) ly_phi[id]=1.;
                                else ly_phi[id]=0.;
                                ly_mu[id]=0.;
                                ...
                                for (ii=0; ii<ndrop; ii++) {
                                    ...
                                    dy = j - drop[ii*3+1];
                                    dz = k - drop[ii*3+2];
                                    if (dx*dx+dy*dy+dz*dz<=rdrop2[ii]) ly_phi[id]=1.-ly_phi[id];
                                }
                            } else if (rand_init==-321) {        // defect annihilation, defect line parallel to far-field director
                                double cx, cy;
                                cx = (double)Nx*0.5;
                                ...
                                ly_phi[id]= 1.;
                                ly_mu[id] = 0.;
                                dx = (double)i - cx + 0.5*q_init;
                                dy = (double)j - cy;
                                if (dx*dx+dy*dy<225.) ly_phi[id]=0.;
                                dx = (double)i - cx - 0.5*q_init;
                                dy = (double)j - cy;
                                if (dx*dx+dy*dy<225.) ly_phi[id]=0.;
                            } else if (rand_init==-411) {        // nematic droplet with smoother director field
                                ...
                                dz = (double)k - Nz/2;
                                dr = sqrt(dy*dy+dz*dz);
                                if (dx*dx+dy*dy+dz*dz<q_init*q_init) ly_phi[id]=1.;
                                else ly_phi[id]=0.;
                                ly_mu[id]=0.;
                            } else if (rand_init==-412) {       // 1 nematic tactoid; long axis = rand_seed; q_init = aspect ratio (>=1)
                                double cx, cy, cz, d, b, c, R, Rsq, a, tx, ty, tz, dx, dy, dz, dr, dr2, dw;
                                ...
                                if (dy*dy+dz*dz>0) {
                                    ...
                                    dz += dz*ir*c;
                                    dr2 = dx*dx+dy*dy+dz*dz;
                                    if (dr2<Rsq) ly_phi[id] = 1.;
                                } else if (fabs(dx)<d) ly_phi[id] = 1.;
                            } else if (rand_init==-421) {       // 1 off-center radial droplet, radius = q_init
                                double cx, cy, cz, dx, dy, dz, dr2;
                                ly_phi[id] = 0.;
                                ly_mu[id]  = 0.;
                                ...
                                dz = (double)k - cz;
                                dr2= dx*dx+dy*dy+dz*dz;
                                if (dr2<=q_init*q_init) ly_phi[id] = 1.;
                            } else if (rand_init==-431) {       // 2 nematic droplets with homeotropic anchoring, radius = q_init
                                double dx, dy, dz, dr2;
                                ...
                                double cx, cy, cz, d, b, c, R, Rsq, a, tx, ty, tz, dx, dy, dz, dr2;
                                ly_phi[id] = 0.;
                                ly_mu[id]  = 0.;
...
//       }
    ...
    if (phi_on!=0) {        // calculate total phi
        ly_phi_tot = 0.;
        for (i=0; i<lpoint; i++) ly_phi_tot += ly_phi[i];
	    MPI_Reduce(&ly_phi_tot, &ly_phi_tot0, 1, MPI_DOUBLE, MPI_SUM, root, MPI_COMM_WORLD);
        if (myid==root) printf("total phi = %e\n", ly_phi_tot0);
    }
}
