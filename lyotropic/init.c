                	...
                	for (j=0; j<Ny; j++) {
                		for (i=0; i<Nx; i++) {
                			...
                			if (rand_init==0 || rand_init==1) {                     // 0: random random seed, 1: random with rand_seed
                                ...
                                if (dr>q_init) ly_phi[id] = 1.;
                                else ly_phi[id] = 0.;
                                ly_mu[id] = 0.;
                			} else if (rand_init==-91) {                             // radial for spherical symmetry + fluctuation
                				double cx, cy, cz, th, dr, idr;
                				cx = p_pos[0]; 
                				cy = p_pos[1];
                				cz = p_pos[2];
                				...
                				if (dr>0) {
                					idr = 1./dr;
                					nx  = dx*idr + 0.015*randvec();
                					ny  = dy*idr + 0.015*randvec();
                					nz  = dz*idr + 0.015*randvec();
                				} else {
                					nx  = 0;
                					ny  = 0;
                					nz  = 0;
                			...
                			} else if (rand_init==-15) {                             // bipolar for spherical symmetry
                				double cx, cy, cz, th, dr, idr, gg;
                				cx = p_pos[0]; 
                				cy = p_pos[1];
                				cz = p_pos[2];
                				dx = (double)i - cx;
                				...
                				dz = (double)k - cz;
                				dr = sqrt(dx*dx+dy*dy);
                                if (p_rad[0]*p_rad[0]-dz*dz>0) {
                                    gg = dr*dr/(p_rad[0]*p_rad[0]-dz*dz);
                                } else gg = 0.;
                				if (dr>0) {
                					idr = 1./dr;
                					nx  =-dx*idr*dz*gg;
                           ...
                           } else if (rand_init==-311) {        // defect annihilation, defect line perpendicular to far-field director
                               double cx, cy, b, th, dx, dy;
                               cx = (double)Nx*0.5;
                               cy = (double)Ny*0.5;
                               b  = q_init;
                               if (b>0.5*cy) b = 0.5*cy;
                               ...
                               dy = (double)j - cy;
                               if (dx*dx+dy*dy<225.) ly_phi[id]=0.;
                            } else if (rand_init==-40) {        // uniform Q, see whether phi will become uniform
                                double dx, dy;
                                nx = 1.;
                                ny = 0.;
                                nz = 0.;
                                dx = (double)i - Nx/2;
                                dy = (double)j - Ny/2;
                                if (dx*dx+dy*dy<q_init*q_init) ly_phi[id]=0.;
                                else ly_phi[id]=1.;
                                ly_mu[id]=0.;
                            } else if (rand_init==-41) {        // nematic droplet
                                double dx, dy, dz;
                                nx = 1.;
                                ny = 0.;
                                nz = 0.;
                                ...
                                ly_mu[id]=0.;
                            } else if (rand_init==-42) {        // nematic ellipsoidal droplet, temperary
                                double dx, dy, dz;
                                nx = 1.;
                                ny = 0.;
                                nz = 0.;
                                ...
                                dy = (double)j - Ny/2;
                                dz = (double)k - Nz/2;
                                if (0.25*dx*dx+dy*dy+dz*dz<q_init*q_init) ly_phi[id]=1.;
                                else ly_phi[id]=0.;
                                ly_mu[id]=0.;
                            }
                            ...
                            // initialize concentration field and chemical potential
                            if (rand_init!=-9 && rand_init!=-15 && rand_init!=-311 && rand_init!=-321 && rand_init!=-40 && rand_init!=-41 && rand_init!=-42 && rand_init!=-9 && rand_init!=-91) {
                                ly_phi[id] = 0.85; 
                                ly_mu[id]  = 0.;
                            }

                            if (rand_init==2 || rand_init==0) {
//                                if (i<Nx/2) ly_phi[id]=0;
//                                else ly_phi[id] = 1.;
                                ly_phi[id]=0.479;
                                ly_mu[id]=0.;
                            }
